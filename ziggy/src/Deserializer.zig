...
pub fn deserialize(
    ...
    errdefer result.arena_state.deinit();
...
pub fn deserializeOne(d: *const Deserializer, T: type, first: Token, top_lvl: bool) Error!T {
    switch (@typeInfo(T)) {
        ...
        .comptime_int, .int => switch (first.tag) {
            .integer => {
                return std.fmt.parseInt(T, first.loc.slice(d.src), 0) catch |err| {
                    switch (err) {
        ...
        inline .array, .vector => |info| {
            ...
            if (info.len == 0) {
                ...
                return if (maybe_rsb.tag == .rsb) undefined else d.lengthMismatch(maybe_rsb);
            }

            var ret: T = undefined;
            for (0..info.len) |i| {
                ret[i] = try d.deserializeOne(
                    info.child,
                    d.next(),
                    false,
                );
                ...
                switch (d.peek()) {
                    ...
                    .rsb => return if (i == info.len - 1) ret else d.lengthMismatch(d.next()),
                    else => return d.unexpected(d.next()),
                }
            }

            const next_tok = d.next();
            if (next_tok.tag != .rsb) return d.lengthMismatch(next_tok);
