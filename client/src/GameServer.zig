...
fn PacketData(comptime tag: std.meta.Tag(network_data.S2CPacket)) type {
    return @FieldType(network_data.S2CPacket, @tagName(tag));
}

fn handlerFn(comptime tag: std.meta.Tag(network_data.S2CPacket)) fn (*Server, PacketData(tag)) void {
    return switch (tag) {
...
fn logRead(comptime tag: std.meta.Tag(network_data.S2CPacket)) bool {
    const name = @tagName(tag);
    const tick = comptime tag == .ping or tag == .show_effects or
...
fn logWrite(comptime tag: std.meta.Tag(network_data.C2SPacket)) bool {
    const tick = tag == .move or tag == .pong;
    return if (tick)
        build_options.log_packets == .all or
...
pub fn allocBuffer(_: [*c]uv.uv_handle_t, _: usize, buf: [*c]uv.uv_buf_t) callconv(.c) void {
    buf.* = .{ .base = @ptrCast(main.game_reader.buffer.ptr), .len = main.game_buffer_size };
}
...
fn newObject(comptime T: type, list: []const network_data.ObjectData, tick_time: f32) void {
    for (list) |obj| {
        ...
        stat_reader.buffer = @constCast(obj.stats);
        stat_reader.reset();
...
fn parsePlayerStat(player: *Player, stat: network_data.PlayerStat) void {
    ...
    switch (stat) {
        ...
        .talents => |val| {
            main.allocator.free(player.talents);
            player.talents = main.allocator.dupe(network_data.DataIdWithCount(u16), val) catch main.oomPanic();
        },
        .rank => |val| player.rank = val,
        .aether => |val| player.aether = val,
        .spirits_communed => |val| {
            const old_spirits = player.spirits_communed;
            player.spirits_communed = val;
            if (old_spirits == std.math.maxInt(u32) or val <= old_spirits) return;
            const texts_len = player.status_texts.items.len;
            const last_add = if (texts_len > 0) player.status_texts.items[texts_len - 1].show_at else -1;
            const cur_time = main.current_time;
            const cooldown: i64 = 0.2 * std.time.us_per_s;
            player.status_texts.append(main.allocator, .{
                .initial_size = 16.0,
                .dispose_text = true,
                .show_at = if (cur_time - last_add >= cooldown) main.current_time else last_add + cooldown,
                .duration = 2.0 * std.time.us_per_s,
                .text_data = .{
                    .text = std.fmt.allocPrint(main.allocator, "+{} &img=\"misc,2\"", .{val - old_spirits}) catch main.oomPanic(),
                    .text_type = .bold,
                    .size = 16,
                    .color = 0xD168FF,
                },
        ...
        .gold => |val| {
            const old_gold = player.gold;
            player.gold = val;
            if (old_gold == std.math.maxInt(u32) or val <= old_gold) return;
            const texts_len = player.status_texts.items.len;
            const last_add = if (texts_len > 0) player.status_texts.items[texts_len - 1].show_at else -1;
            const cur_time = main.current_time;
            const cooldown: i64 = 0.5 * std.time.us_per_s;
            player.status_texts.append(main.allocator, .{
                ...
                .duration = 2.0 * std.time.us_per_s,
                .text_data = .{
                    .text = std.fmt.allocPrint(main.allocator, "+{} &img=\"misc,0\"", .{val - old_gold}) catch main.oomPanic(),
                    .text_type = .bold,
                    .size = 16,
                    .color = 0xEFB564,
                },
        ...
        .gems => |val| {
            const old_gems = player.gems;
            player.gems = val;
            if (old_gems == std.math.maxInt(u32) or val <= old_gems) return;
            const texts_len = player.status_texts.items.len;
            const last_add = if (texts_len > 0) player.status_texts.items[texts_len - 1].show_at else -1;
            const cur_time = main.current_time;
            const cooldown: i64 = 0.5 * std.time.us_per_s;
            player.status_texts.append(main.allocator, .{
                ...
                .duration = 2.0 * std.time.us_per_s,
                .text_data = .{
                    .text = std.fmt.allocPrint(main.allocator, "+{} &img=\"misc,1\"", .{val - old_gems}) catch main.oomPanic(),
                    .text_type = .bold,
                    .size = 16,
                    .color = 0x77F745,
                },
