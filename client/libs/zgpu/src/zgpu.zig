...
pub const GraphicsContext = struct {
    ...
    pub fn create(
        ...
        std.log.info("High performance device has been selected:", .{});
        std.log.info("- Device: {s}", .{props.name});
        std.log.info("- Driver: {s}", .{props.driver_description});
        std.log.info("- Adapter: {s}", .{@tagName(props.adapter_type)});
        std.log.info("- Backend: {s}", .{@tagName(props.backend_type)});
...
pub fn bufferEntry(
    ...
    has_dynamic_offset: wgpu.WGPUBool,
...
fn logUnhandledError(
    ...
    _ = userdata;
    switch (err_type) {
        .no_error => std.log.info("No error: {?s}", .{message}),
        .validation => std.log.err("Validation error: {?s}", .{message}),
        .out_of_memory => std.log.err("Out of memory: {?s}", .{message}),
        .device_lost => std.log.err("Device lost: {?s}", .{message}),
        .internal => std.log.err("Internal error: {?s}", .{message}),
        .unknown => std.log.err("Unknown error: {?s}", .{message}),
    }
