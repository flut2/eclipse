...
tiles: std.ArrayList(network_data.TileData) = .empty,
tiles_seen: std.AutoHashMapUnmanaged(u32, u16) = .empty,
objs: struct {
    entity: std.ArrayList(network_data.ObjectData) = .empty,
    enemy: std.ArrayList(network_data.ObjectData) = .empty,
    player: std.ArrayList(network_data.ObjectData) = .empty,
    portal: std.ArrayList(network_data.ObjectData) = .empty,
    container: std.ArrayList(network_data.ObjectData) = .empty,
    ally: std.ArrayList(network_data.ObjectData) = .empty,
} = .{},
drops: struct {
...
show_effects: std.ArrayList(network_data.ShowEffectItem) = .empty,
...
pub fn damage(
    ...
    if (dmg > 0) {
        ...
        if (self.ability_state.heart_of_stone and self.keystoneTalentLevel(1) > 0) self.stored_damage += @intCast(dmg);
    }
