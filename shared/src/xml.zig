...
pub const Doc = struct {
    ...
    // Must handle freeing yourself
    pub fn toMemory(self: Doc, allocator: std.mem.Allocator) ![]const u8 {
        var ptr: ?[*]c.xmlChar = undefined;
        var size: c_int = undefined;
        c.xmlDocDumpMemory(self.impl, &ptr, &size);
        if (ptr) |xml_ptr| {
            defer c.xmlFree.?(xml_ptr);
            return try allocator.dupe(u8, xml_ptr[0..@intCast(size)]);
        } else return error.BadAlloc;
    }
...
pub const DocWriter = struct {
    impl: c.xmlTextWriterPtr,
    doc: c.xmlDocPtr,

    pub fn create() DocWriter {
        var writer: DocWriter = undefined;
        writer.impl = c.xmlNewTextWriterDoc(&writer.doc, 0);
        return writer;
    }

    pub fn childDoc(self: DocWriter) Doc {
        return .{ .impl = self.doc };
    }

    pub fn deinit(self: DocWriter) void {
        c.xmlFreeTextWriter(self.impl);
        c.xmlFreeDoc(self.doc);
    }

    pub fn startDocument(self: DocWriter, opts: struct {
        version: [*c]const u8 = null,
        encoding: [*c]const u8 = null,
        standalone: [*c]const u8 = null,
    }) !void {
        if (c.xmlTextWriterStartDocument(self.impl, opts.version, opts.encoding, opts.standalone) == -1)
            return error.InvalidDocument;
    }

    pub fn endDocument(self: DocWriter) !void {
        if (c.xmlTextWriterEndDocument(self.impl) == -1)
            return error.InvalidDocument;
    }

    pub fn startElement(self: DocWriter, elem_name: [:0]const u8) !void {
        if (c.xmlTextWriterStartElement(self.impl, elem_name) == -1)
            return error.InvalidElement;
    }

    pub fn writeElement(self: DocWriter, elem_name: [:0]const u8, elem_content: [:0]const u8) !void {
        if (c.xmlTextWriterWriteElement(self.impl, elem_name, elem_content) == -1)
            return error.InvalidElement;
    }
    ...
    pub fn startAttribute(self: DocWriter, attrib_name: [:0]const u8) !void {
        if (c.xmlTextWriterStartAttribute(self.impl, attrib_name) == -1)
            return error.InvalidAttribute;
    }

    pub fn writeAttribute(self: DocWriter, attrib_name: [:0]const u8, attrib_content: [:0]const u8) !void {
        if (c.xmlTextWriterWriteAttribute(self.impl, attrib_name, attrib_content) == -1)
            return error.InvalidAttribute;
    }

    pub fn endAttribute(self: DocWriter) !void {
        if (c.xmlTextWriterEndAttribute(self.impl) == -1)
            return error.InvalidAttribute;
    }
};
