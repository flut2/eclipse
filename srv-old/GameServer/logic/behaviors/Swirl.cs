...
internal class Swirl : CycleBehavior
{
    ...
    protected override void TickCore(Entity host, RealmTime time, ref object state)
    {
        ...
        var period = (long)(1000 * radius / host.GetSpeed(speed) * (2 * Math.PI) * 1000);
        if (!s.Acquired &&
            s.RemainingTime <= 0 &&
            targeted)
        {
        ...
        else if (s.RemainingTime <= 0 || (s.RemainingTime - period > 200 && host.GetNearestEntity(2, null) != null))
        {
            if (targeted)
            {
                ...
                if (entity != null)
                    ...
                    s.RemainingTime = 5000 * 1000;
            }
            else
                s.RemainingTime = 5000 * 1000;
        }
        else
            s.RemainingTime -= time.ElapsedUsDelta;
        ...
        angle += angularSpd * (time.ElapsedUsDelta / (1000f * 1000f));
        ...
        vect.Normalize();
        vect *= (float)spd * (time.ElapsedUsDelta / (1000f * 1000f));
